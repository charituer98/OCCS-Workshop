...
7.  “server-jre-8u121-linux-x64.tar.gz”을 Docker 호스트에 업로드 하십시오,
    경로는
...
8.  Docker 호스트에 “fmw_12.2.1.0.0_wls_quick_Disk1_1of1.zip” 을 업로드
    하십시오, 경로는
...
2.  WebLogic Docker 이미지는 이전 JRE를 기반으로 하며 다음 명령어를 실행합니다.
...
3.  이전의 Weblogic Docker 이미지를 도메인이있는 새 이미지로 확장하고 다음
    명령을 실행합니다
...
4.  새로운 도메인 이미지를 다시 확장하고 최신 이미지에 간단한 데모 Application이
...
5.  로드밸런서 Docker 이미지는 runit 과 confd 를 기반으로 하며 Container Cloud에
...
6.  이 시점에서, Java 와 WebLogic 이미지를 로드 밸런서 이미지와 함께 빌드하면
    다음과 같이 표시되어야 합니다.
...
3.  위에 설명한 과정을 반복하여 nginx-lb 이미지를 Docker hub에 푸시합니다.
...
4.  새 브라우저 탭을 열고 URL(<http://hub.docker.com> ) 입력하고 Docker hub 계정
...
2.  좌측 메뉴의 “Stacks”을 클릭, 모든 stack 목록이 오른쪽 패널에 있습니다.

![](media/38761fab21c7b9f8d91b4bcf69c0c6ff.png)

3.  오른쪽 상단의 “New Stack” 클릭 , “WLS-multi-host-LB”라는 새로운 Stack이
    생성됩니다.

![](media/d130c43589f9aee5df831eae5b4889d8.png)

4.  “Docker Compose” 포맷과 유사한 Stack editor인 “Advanced Editor”를 클릭
    합니다.

![](media/f66fc2bc621d213e0c0ab84b234113ed.png)

5.  Copy and paste 다음 라인들을 Editor에 복사하고 붙여 넣으세요.
...
6.  “Done” 을 클릭해서 편집기를 닫으세요

![](media/edd142eeb9ff3998ff82f1ae00090ddc.png)

7.  Stack name : WLS-multi-host-LB 입력, 그리고 “Save” 클릭

![](media/605004c09136cbc6b6780f6d9ded3eed.png)

8.  새로운 Stack이 성공적으로 빌딩 되었다.
...
2.  필요로 하는 managed servers를 입력하세요, 예에서는 2를 선택하고 “Deploy”
    클릭

![](media/f13bea90dd6ed49dd04be7d24529a21d.png)

3.  동작하기 위한 새로운 배포가 이미 시작되고 Health-check를 통해서 새로 배포된
...
4.  몇 분후에, Admin server 이미 실행 중입니다.

![](media/66586ddb89ed7a448854ca20a377978a.png)

5.  Public IP 주소를 얻기 위해 Host인 “tutorial-instance-occs-wkr-1” 를
    클릭합니다.

![](media/64a056517c93128e18a68e8f86b48a0d.png)

6.  Open a new browser tab with the public IP + predetermined port 8001 이 admin
...
7.  deployment로 돌아가면 2개의 managed server와 nginx 로드밸런서가
    health-checks를 통과하여 running 중임을 확인할 수 있습니다.

![](media/b9256638544001c72b057633175c03a2.png)

8.  새로운 브라우저 탭을 열고 managed server 응답을 확인 하십시오.

>   url 입력:

![](media/fe30fb2cfc64d0ab8aaf00a1543a790e.png)

9.  페이지 응답은 managed server 1 에서 발생하며 , F5를 누르면 페이지가 새로
...
2.  우리가 원하는 대로 managed servers 수를 변경할 수 있습니다, 이 예제에서는
    1개의 서버를 추가할 것입니다.

![](media/479018647d89d23a0a22c4617e8a7e71.png)

3.  스케일 아웃 수행을 위해 “Change”를 클릭합니다.

![](media/6103674ca32af02606647cd73f16fa00.png)

4.  몇 초후에 , 세번째 managed server 인스턴스가 준비되어야 합니다, 마지막으로
    모든 서버가 실행됩니다.
